...
class OptionsHandler {
    ...
    static check(options?: IOptions) {
        ...
        if (options.provider && !(options.provider instanceof providers[Object.keys(providers)[0]].constructor)) throw new Error("Provider option must be valid. Try { provider: (new FreeGPT()).providers.ChatBase, ... }");
        if (options.retry) {
